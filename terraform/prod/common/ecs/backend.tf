...
## partner backend
resource "aws_ecs_task_definition" "spring-gateway" {
  family                   = var.spring.gateway.task_definition_name
  requires_compatibilities = ["FARGATE"]
  task_role_arn            = var.spring.gateway.task_role_arn
  execution_role_arn       = var.spring.gateway.execution_role_arn
  ...
  tags = {
    Environment = var.env
    Terraform   = "true"
    Project     = var.spring.common.project
    Component   = var.spring.gateway.dd_service
  }
  ...
  container_definitions = jsonencode(flatten([
    ...
    [
      {
        name         = "app"
        image        = var.spring.gateway.container_info.app_image
        portMappings = var.spring.gateway.container_info.app_ports
        linuxParameters = {
          initProcessEnabled = true
        }
        dockerLabels = merge(var.spring.gateway.container_info.app_dd_labels, {
        ...
        logConfiguration = {
          ...
          options = {
            ...
            apikey         = var.dd_config.api_key
            provider       = "ecs"
            dd_service     = var.spring.gateway.dd_service
            Host           = "http-intake.logs.datadoghq.com"
            TLS            = "on"
            dd_source      = "spring-gateway"
            dd_tags        = "project:${var.spring.common.project},env:${var.env}"
            Name           = "datadog"
          }
        },
      },
      {
        name  = "eureka"
        image = var.spring.gateway.container_info.eureka_image
        portMappings = [
          {
            containerPort = 8761
            hostPort      = 8761
        ...
        dockerLabels = merge(var.spring.gateway.container_info.app_dd_labels, {
          "com.datadoghq.tags.project" = var.spring.common.project,
          "com.datadoghq.tags.env"     = var.env
        })
      ...
      {
        name  = "nginx"
        image = var.spring.gateway.container_info.nginx_image
        portMappings = [
          {
            containerPort = 80
            hostPort      = 80
...
resource "aws_ecs_service" "spring-gateway" {
  name                   = var.spring.gateway.ecs_service_name
  cluster                = var.spring.common.cluster_name
  task_definition        = aws_ecs_task_definition.spring-gateway.arn
  desired_count          = 1
  launch_type            = "FARGATE"
  enable_execute_command = true
  ...
  network_configuration {
    subnets         = data.terraform_remote_state.vpc.outputs.private_subnets
    security_groups = [data.aws_security_group.spring_gateway.id]
  }
