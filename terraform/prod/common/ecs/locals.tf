locals {
  ...
  partner_backend_dd_docker_labels = {
    dockerLabels = {
      "com.datadoghq.tags.service" = var.spring.gateway.dd_service
      "com.datadoghq.tags.env"     = var.env
    }
  }
  ...
  { name = k, value = v }]
  partner_backend_dd_container_env = [for k, v in merge(local.dd_container_env, { DD_SERVICE = var.spring.gateway.dd_service }) :
  { name = k, value = v }]
...
data "aws_security_group" "spring_gateway" {
  filter {
    name   = "group-name"
    values = ["gateway"]
  }
...
#  filter {
#    name   = "group-name"
#    values = ["beacon-frontend00000000000000000000000002"]
#  }
#}
# beacon-scan
