...
locals {
  ...
  second_image_url = "${local.account_id}.dkr.ecr.${local.region}.amazonaws.com/${var.second_image_name}"
...
resource "aws_ecr_repository" "second_main" {
  name = var.second_image_name
  tags = var.tags
    ...
    },
    {
      ...
      logConfiguration = {
        ...
        options = {
          ...
          Host           = "http-intake.logs.datadoghq.com"
          TLS            = "on"
          dd_service     = var.dd_second_service
          dd_source      = var.dd_second_source
          dd_message_key = "log"
          dd_tags        = "project:${var.dd_project}, env:${var.dd_env}"
          provider       = "ecs"
        }
