locals {
  ...
  beacon_image_name     = "cloud/auth${local.postfix_slash}"
  beacon_image_url      = join("/", [local.image_url_prefix, local.beacon_image_name])
  beacon_image_tag      = "latest"
  beacon_container_name = "auth${local.postfix_hyphen}"
  beacon_container_port = 8091

  dd_project = "partner${local.postfix_hyphen}"
  dd_service = "auth${local.postfix_hyphen}"
  dd_source  = "java"
