locals {
  ...
  image_name     = "partner/management/back${local.postfix_slash}"
  image_url      = join("/", [local.image_url_prefix, local.image_name])
  image_tag      = "latest"
  container_name = "partner-management-back${local.postfix_hyphen}"
  container_port = 8080
  ...
  dd_project = "hispital${local.postfix_hyphen}"
  dd_service = "management-back${local.postfix_hyphen}"
  dd_source  = "java"
